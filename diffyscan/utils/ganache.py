...
class Ganache:
  ...
  def start(self):
      ...
      if (is_port_used):
        ...
        if (answer.lower() == 'yes'):
            return_code = subprocess.call(f'exec npx kill-port {parsed_url.port}', shell=True)
            if (return_code == 0):
              is_port_used = self.is_port_in_use(parsed_url)
      if (is_port_used):
        raise ValueError(f'Failed to start Ganache: {parsed_url.netloc} is busy')
      self.sub_process = subprocess.Popen("exec "+ local_node_command, shell=True, stdout=subprocess.DEVNULL, stderr=subprocess.PIPE)
      try:
